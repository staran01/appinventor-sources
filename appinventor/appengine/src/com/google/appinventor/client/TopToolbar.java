...
public class TopToolbar extends Composite {
	...
	public TopToolbar() {
		...
		connectItems.add(new DropDownItem(WIDGET_NAME_WIRELESS_BUTTON, MESSAGES
				.wirelessButton(), new WirelessAction()));
		connectItems.add(new DropDownItem(WIDGET_NAME_SMARTLAB_BUTTON, MESSAGES
				.smartlabButton(), new SmartlabAction()));
		connectItems.add(new DropDownItem(WIDGET_NAME_EMULATOR_BUTTON, MESSAGES
				.emulatorButton(), new EmulatorAction()));
		connectItems.add(new DropDownItem(WIDGET_NAME_USB_BUTTON, MESSAGES
				.usbButton(), new UsbAction()));
		connectItems.add(null);
		connectItems.add(new DropDownItem(WIDGET_NAME_RESET_BUTTON, MESSAGES
				.resetConnections(), new ResetAction()));
		...
		buildItems.add(new DropDownItem(WIDGET_NAME_BUILD_BARCODE, MESSAGES
				.showBarcodeButton(), new BarcodeAction()));
		buildItems.add(new DropDownItem(WIDGET_NAME_BUILD_DOWNLOAD, MESSAGES
				.downloadToComputerButton(), new DownloadAction()));
		if (AppInventorFeatures.hasYailGenerationOption()
			...
			buildItems.add(new DropDownItem(WIDGET_NAME_BUILD_YAIL, MESSAGES
					.generateYailButton(), new GenerateYailAction()));
		}
		...
		if (Ode.getInstance().getUser().getIsAdmin()) {
			...
			adminDropDown = new DropDownButton(WIDGET_NAME_ADMIN,
					MESSAGES.adminButton(), adminItems, false);
	...
	private class SmartLabDialog extends DialogBox {

		private TextBox username;
		private TextBox key;

		public SmartLabDialog() {
			// Set the dialog box's caption.
			super(false, true);
			setText("SmartLab Authentication");
			...
			apikeyLabel.setWidth("100px");
			apikeyLabel.setHeight("40px");

			hpanel2.add(apikeyLabel);
			...
			TextBox apikey = new TextBox();
			// apikey.setText("");
			apikey.setWidth("150px");
			apikey.setHeight("20px");
			this.key = apikey;
			hpanel.add(username);
			hpanel2.add(apikey);
			...
			Button ok = new Button("Authenticate");
			ok.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					SmartLabDialog.this.hide();
					ProjectRootNode projectRootNode = Ode.getInstance()
							.getCurrentYoungAndroidProjectRootNode();
					if (projectRootNode != null) {
						String target = YoungAndroidProjectNode.YOUNG_ANDROID_TARGET_ANDROID;
						ChainableCommand cmd = new SaveAllEditorsCommand(
								new GenerateYailCommand(new BuildCommand(
										target, new ShowProgressBarCommand(
												target,
												new WaitForBuildResultCommand(
														target,
														new SmartLabCommand(
																target,getUsername(),getKey()),
														"SmartLabAction"),
												"SmartLabAction"),
										"SmartLabAction")));
						// updateBuildButton(true);
						cmd.startExecuteChain(
								Tracking.PROJECT_ACTION_BUILD_BARCODE_YA,
	...
	private class SmartlabAction implements Command {
		@Override
		public void execute() {
			new SmartLabDialog().show();

		}
		/*
		 * public void execute(){
		 * //System.out.println("Smartlab action clicked!");
		 * startRepl(false,false,false,true);//need to change the true false
		 * true booleans so it accepts a new command new
	...
	private class ResetAction implements Command {
		@Override
		public void execute() {
			startRepl(false, false, false, false); // We are really stopping the
													// repl here
		}
	}
	...
	private class BarcodeAction implements Command {
		...
		public void execute() {
			...
			if (projectRootNode != null) {
				String target = YoungAndroidProjectNode.YOUNG_ANDROID_TARGET_ANDROID;
				ChainableCommand cmd = new SaveAllEditorsCommand(
						new GenerateYailCommand(new BuildCommand(target,
	...
	private static class ExportProjectAction implements Command {
		...
		public void execute() {
			...
			if (selectedProjects.size() == 1) {
				...
				ErrorReporter
						.reportInfo(MESSAGES.wrongNumberProjectsSelected());
			}
		}
	...
	private static class DeleteAction implements Command {
		...
		private boolean deleteConfirmation(List<Project> projects) {
			String message;
			if (projects.size() == 1) {
				message = MESSAGES.confirmDeleteSingleProject(projects.get(0)
						.getProjectName());
			} else {
	...
	private void startRepl(boolean start, boolean forEmulator, boolean forUsb,
		...
		if (start) {
			if (forEmulator) { // We are starting the emulator...
				...
				updateConnectToDropDownButton(false, false, false, true);// need
																			// to
																			// add
																			// new
																			// boolean
	...
	public void updateFileMenuButtons(int view) {
		if (view == 0) { // We are in the Projects view
			fileDropDown.setItemEnabled(MESSAGES.deleteMenuItemButton(), Ode
					.getInstance().getProjectManager().getProjects() == null);
			fileDropDown
					.setItemEnabled(MESSAGES.exportAllProjectsButton(), Ode
			...
			buildDropDown.setItemEnabled(MESSAGES.downloadToComputerButton(),
					false);
				...
				.hasUserFile(StorageUtil.ANDROID_KEYSTORE_FILENAME,
						new AsyncCallback<Boolean>() {
							...
							public void onFailure(Throwable caught) {
								// Enable the buttons. If they are clicked,
								// we'll check again if the keystore exists.
								fileDropDown.setItemEnabled(
										MESSAGES.deleteKeystoreButton(), true);
								fileDropDown.setItemEnabled(
										MESSAGES.downloadKeystoreButton(), true);
							}
						});
	}
